...
<div id="content-wrapper">
    ...
    <div  class="displayCellHead">User name:</div><%
 ...
 //Main select and assign all main variables 
        ...
        {
            int [] userAccess = myMainSelect.obtainUserAccess(users[i]);
                    
            
         %><div  class="displayRow"><div  class="displayCell"><a href="../modify?userModify=<%=users[i]%>"><%=users[i]%> </a></div><%
            for (int k = 0; k < classNames.length; k++)
            {
                if (userAccess[k] >2) {userAccess[k] = 0;}
                %><div class="displayCell"><%=classAccess[userAccess[k]]%></div><%
            }
            %></div><%     
        }
