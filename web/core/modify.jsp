...
<div id="content-wrapper">
    ...
    <!-- Table start -->
    <div style="margin: 10px;"><font color="#0095d3" style="font-weight: bold;">Modifying: <%=userModify%></font></div>
    <div class="displayTable">
   <form action="../updateUserAccess" method="post">        
        <div class="displayRow">
         
    <div  class="displayCellHead">Access:</div><%

    for (int i=0; i < classNames.length; i++)
    {
        %><div class="displayCellHead"><%=classNames[i]%></div><%
    }
    %></div><%
    
 //Main select and assign all main variables 
        sqlSelect myMainSelect = new sqlSelect();
         String [] users = myMainSelect.obtainUsersList();
        
        for (int i=0; i < classAccess.length; i++)
        {
            int [] userAccess = myMainSelect.obtainUserAccess(userModify);
                    
         %><div  class="displayRow"><div  class="displayCell"> <%=classAccess[i]%></div><%
            for (int k = 0; k < classNames.length; k++)
            {
                if (userAccess[k] >2) {userAccess[k] = 0;}
                %><div class="displayCell"><input type="checkbox" name="<%=classNames[k]%><%=classAccess[i]%>"<% if (classAccess[i] == classAccess[userAccess[k]]) { %>checked<% }%>></div><%
            }
            %></div><%     
        }
...
<!-- Table ends -->        
    ...
    <div style="padding-left: 20px;"><input type="submit" value="Change" class="submitButton"></div>
    </form>
<!-- networkInfo site A
