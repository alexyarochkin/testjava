...
public class updateUserAccess extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        {
        String userModify = request.getSession(false).getAttribute("userModify").toString();
...
//        String classNames[] = {"vpn","firewall", "network", "linux", "windows", "vmware", "ups", "cloud", "printers", "applications", "serviceAccount"};
        ...
        int [] userAccess;
        userAccess = new int[classNames.length];
        ...
        for (int i=0; i < classAccess.length; i++)
        {
            ...
            {
                if (request.getParameter(classNames[k]+classAccess[i]) != null)
                {
                    userAccess[k]=i;
                }
            }
       ...
       myUpdate.sqlUpdateUserAccess(userAccess, userModify);
        
        
        response.sendRedirect("core/modify.jsp");
        }
        else 
        {
